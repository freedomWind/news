...
public class ResourceUpdateProcedure : GameBaseProcedure
{
    ...
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        ...
        EntityInst.Event.Subscribe(ResourceUpdateStartEventArgs.EventId, OnResourceUpdateStart);
        EntityInst.Event.Subscribe(ResourceUpdateChangedEventArgs.EventId, OnResourceUpdateChanged);
        EntityInst.Event.Subscribe(ResourceUpdateSuccessEventArgs.EventId, OnResourceUpdateSuccess);
        EntityInst.Event.Subscribe(ResourceUpdateFailureEventArgs.EventId, OnResourceUpdateFailure);

        //if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork)
        //{
        //    FrameworkEntry.UI.OpenDialog(new DialogParams
        //    {
        //        Mode = 2,
        //        Title = "UpdateResourceViaCarrierDataNetwork.Title",
        //        Message = "UpdateResourceViaCarrierDataNetwork.Message",
        //        ConfirmText = "UpdateResourceViaCarrierDataNetwork.UpdateButton",
        //        OnClickConfirm = StartUpdateResources,
        //        CancelText = "UpdateResourceViaCarrierDataNetwork.QuitButton",
        ...
        {
            procedureOwner.SetData<VarGameObject>("UpdateResourceForm", m_UpdateResourceForm.gameObject);
        }
    }

    protected override void OnLeave(IFsm<IProcedureManager> procedureOwner, bool isShutdown)
    {
        EntityInst.Event.Unsubscribe(ResourceUpdateStartEventArgs.EventId, OnResourceUpdateStart);
        EntityInst.Event.Unsubscribe(ResourceUpdateChangedEventArgs.EventId, OnResourceUpdateChanged);
        EntityInst.Event.Unsubscribe(ResourceUpdateSuccessEventArgs.EventId, OnResourceUpdateSuccess);
        EntityInst.Event.Unsubscribe(ResourceUpdateFailureEventArgs.EventId, OnResourceUpdateFailure);

        base.OnLeave(procedureOwner, isShutdown);
    }

    protected override void OnNewUpdate(IFsm<IProcedureManager> procedureOwner, float elapseSeconds,
        float realElapseSeconds)
    {
        ...
        GameObject.Destroy(m_UpdateResourceForm.gameObject);
    ...
    private void StartUpdateResources(object userData)
    {
        if (m_UpdateResourceForm == null)
        {
            m_UpdateResourceForm = Object.Instantiate(EntityInst.BuiltinData.UpdateResourceFormTemplate);           
        }

        Log.Info("Start update resources...");
        EntityInst.Resource.UpdateResources(OnUpdateResourcesComplete);
    }
    ...
    private void RefreshProgress()
    {
        ...
        string descriptionText = $"正在为您下载资源包:  <color=#ffe708>{GetByteLengthString(currentTotalUpdateLength)}</color>/{GetByteLengthString(m_UpdateTotalCompressedLength)}  速度: {GetByteLengthString((int)EntityInst.Download.CurrentSpeed)}/s";
        m_UpdateResourceForm.SetUpdateProgress(progressTotal, descriptionText,(m_UpdateTotalCompressedLength - currentTotalUpdateLength)/ EntityInst.Download.CurrentSpeed);
    }
